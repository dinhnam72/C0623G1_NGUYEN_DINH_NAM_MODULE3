...
VALUES (3,3,57,14);       
 ...
 WHERE credit = ( SELECT MAX(credit)
				FROM subjects);
 SELECT  sj.sub_id, sj.sub_name,sj.credit, sj.status_subject, MAX(IFNULL(mark,0)) mark
 FROM subjects sj
 LEFT JOIN mark m ON m.sub_id = sj.sub_id
 GROUP BY sj.sub_id;
 
 SELECT sd.student_id, sd.student_name, AVG(mark) medium_mark
 FROM student sd
 LEFT JOIN mark m ON sd.student_id = m.student_id
